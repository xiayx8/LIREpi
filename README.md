...
```python
python piston/map_label/compute_label3.py
```
Calculate the atomic distance between two chains to obtain the epitope label, preparing for the input of subsequent code scripts.
```python
python piston/map_label/sema_map_atom1.py
```
Standardize the image size to prepare for the input of the subsequent ViT model.
```python
python piston/get_distance_atom.py
```
Calculate the spatial proximity distance.
```python
python piston/6fea-13fea2.py
```
Merge the six-dimensional features of the two chains with the spatial proximity distance features into a 13-dimensional image.
```python
python piston/muti_piston_vector3.py
```
Extract protein structural features through the Vision Transformer model. 
