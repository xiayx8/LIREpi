...
from .get_structure import crop_pdb_antigen,crop_pdb_antibody, download
from .triangulate import triangulate_antigen,triangulate_antibody
from .compute_patches import compute_patches
from .convert_to_images import convert_antigen_to_images
from .convert_to_images import convert_antibody_to_images
from .map_patch_atom import map_patch_atom
from .get_antibody_cdr import download_antibody_cdr
...
def triangulate_unrefined_antigen(ppi, config):
    print("now triangulate the unrefined pdb document")
    triangulate_antigen(ppi, config, overwrite=False)

def triangulate_unrefined_antibody(ppi, config):
    print("now triangulate the unrefined pdb document")
    triangulate_antibody(ppi, config, overwrite=False)

def preprocess_worker(ppi, config):
    config['ppi_const']['patch_r'] = config['ppi_const']['init_r']
    antigen_ppi = ppi.split(",")[0]
    antigen_pid = antigen_ppi.split('_')[0]
    antibody_ppi = ppi.split(",")[1]
    antibody_pid = antibody_ppi.split("_")[0]
    out_grid_antigen = config['dirs']['grid'] + '/' + antigen_ppi + '.npy'
    out_grid_antibody = config['dirs']['grid'] + '/' + antibody_ppi + '.npy'
    out_feature_antigen = config['dirs']['patches'] + '/' + antigen_pid + '/' + antigen_ppi + '_map.csv'
    out_feature_antibody = config['dirs']['patches'] + '/' + antibody_pid + '/' + antibody_ppi + '_map.csv'
    ...
    if os.path.exists(out_feature_antigen) and os.path.exists(out_feature_antibody):#特征文件存在
        print("feature already exists, just do the convert to images part,now convert antigen to images")
        success = False
        config['ppi_const']['patch_r'] = config['ppi_const']['init_r']#patch_r=32
        for _ in range(15):#重试15次
            try:
                convert_antigen_to_images([ppi], config,flag = False)
                success = True
                break
            except Exception as e:
                print(f"WARNING::Couldn't convert to image for {ppi}.")
                print("can not map all")
                config['ppi_const']['patch_r'] += 4
                print(f"Exception details: {e}")

        if not success:
            convert_antigen_to_images([ppi], config,flag = True)
            # 写入文件
            with open("fail.txt", "a") as f:
                f.write(ppi + "\n")

        print("feature already exists, just do the convert to images part,now convert antibody to images")
        success = False
        config['ppi_const']['patch_r'] = config['ppi_const']['init_r']
        for _ in range(15):
            try:
                convert_antibody_to_images([ppi], config, flag=False)
                success = True
                break
            except Exception as e:
                print(f"WARNING::Couldn't convert to image for {ppi}.")
        ...
        if not success:
            convert_antibody_to_images([ppi], config, flag=True)
            # 写入文件
            with open("fail.txt", "a") as f:
                f.write(ppi + "\n")
        ...
        try:
            download_antibody_cdr(ppi,config)
        except Exception as e:
            with open("fail.txt","a") as f:
                f.write(ppi+"\n")
            print(f"WARNING::step get cdr has failed for {ppi}, please check the cdr code.")
            return

        try:
            crop_pdb_antigen([ppi], config, use_refined=False)
            crop_pdb_antibody([ppi], config, use_refined=False)
        except Exception as e:
            with open("fail.txt", "a") as f:
                f.write(ppi + "\n")
            print(f"WARNING::step crop has failed for {ppi}, please check the crop code.")
            return
        try:
            triangulate_unrefined_antigen(ppi, config)
            triangulate_unrefined_antibody(ppi, config)
        except Exception as e:
            with open("fail.txt", "a") as f:
                f.write(ppi + "\n")
            print(f"WARNING::step triangulate has failed for {ppi}, please check the triangulate code.")
            print(e)
            return
        

        try:
            compute_patches([ppi], config, overwrite=False)
        except Exception as e:
            with open("fail.txt", "a") as f:
                f.write(ppi + "\n")
            print(f"WARNING::the step compute patches has failed for {ppi}, please check the correspondingly code.")
            print(e)
            return
        ...
        try:
            ...
            print(f"{e}")
            return
        print("now convert antigen to images")
        success = False
        config['ppi_const']['patch_r'] = config['ppi_const']['init_r']
        for _ in range(15):
            try:
        ...
        print("now convert antibody to images")
        success = False
        config['ppi_const']['patch_r'] = config['ppi_const']['init_r']
        for _ in range(15):
            try:
