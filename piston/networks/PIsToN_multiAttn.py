...
class PIsToN_multiAttn(nn.Module):
    ...
    def forward(self, img, energies, labels=None):
        ...
        #print('x = torch.stack(all_x, dim=1)',x.shape)
        ...
        # print('经过x, feature_attn = self.feature_transformer(x)后')
        # print('x',x.shape)
        # print('feature attn长度',len(feature_attn))
        # print('第一个feature attn大小', feature_attn[0].shape)
        x = x[:, 0]
        # print('x = x[:, 0]',x.shape)
        ...
        # logits = torch.stack(
        #     [dist_to_pos_prototype - dist_to_neg_prototype, dist_to_neg_prototype - dist_to_pos_prototype], axis=1)
        #
    ...
    #         #print('i',i)
    #         img_tmp = img[:,self.index_dict[feature][0],:,:]
    #         #print('img_temp',img_tmp.shape)
    #         energy_tmp = energies[:, self.index_dict[feature][1]]
    #         #print('energy_tmp',energy_tmp.shape)
    #         x, attn = self.spatial_transformers_list[i](img_tmp, energy_tmp)
    #         #print('x, attn = self.spatial_transformers_list[i](img_tmp, energy_tmp)',x.shape)#(1,64,16)
    #         all_x.append(x)
    #         all_spatial_attn.append(attn)
    #     # print(len(all_x))#all_x长度是5，其中每一个的大小是（1,64,16）
    #     # print(all_x[0].shape)
    #     # 使用 torch.mean() 对 all_x 中的张量沿第 0 维（len(all_x) 维度）进行均值计算，均值方法——————————————
    #     # mean_x = torch.mean(torch.stack(all_x), dim=0)
    #     #
    #     # x = mean_x[:, 1:, :]
    #     #————————————————————————————————————————————————————
    #     # Step 1: 对 all_x 中的数据进行加和，加和max-min normalization方法
    #     summed_x = torch.sum(torch.stack(all_x), dim=0)
    #     #print(summed_x.shape)
    #     summed_x = summed_x[:, 1:, :]
    #     #print(summed_x.shape)
    #     # Step 2: Min-Max Normalization
    #     min_x = summed_x.min()  # 获取最小值
    #     max_x = summed_x.max()  # 获取最大值
    #     normalized_x = (summed_x - min_x) / (max_x - min_x)  # Min-Max 归一化
    #
    #     # 确保输出的大小是 (1, 64, 16)
    #     x = normalized_x # 让其变为 (1, 64, 16) 的大小
    #     return x,all_spatial_attn
    #
